 ...
 */
	...
	$.jstree.core.prototype = {
		 ...
		 */
		trigger : function (ev, data, callback) {
			if(!data) {
				data = {};
			}
			data.instance = this;
			this.element.triggerHandler(ev.replace('.jstree','') + '.jstree', data);
			if(callback) { callback.call(this, ev, data); }
		},
		/**
...
//						$('#event_result').html('Test  by nodeId : ' + fdata.node.text );
		...
		rename_node : function (obj, val) {
			 ...
			 */
			this.trigger('rename_node', { "node" : obj, "text" : val, "old" : old }, function(action, data) {
...
//							$('#event_result').html('tmp : ' + tmp.text + ' nv : ' + nv + ' cancel : ' + cancel + ' default_text : ' + default_text);
							if(callback) {
								callback.call(this, tmp, nv, cancel, default_text);
							}
						}, this),
						"keydown" : function (e) {
							var key = e.which;
			...
			//$('#event_result').html('inside edit : ' + t.text);
		},
					...
					*/
					"action"			: function (data) {
						var inst = $.jstree.reference(data.reference),
							obj = inst.get_node(data.reference);
						inst.edit(obj);
/*						inst.edit(obj,false,function(tmp, nv, cancel, default_text){
							$.ajax({
								type:"post",
								url:"renameResultSaved",
								dataType:"json",
								data: {	"old" : default_text, "renamed" : tmp.text},
								timeout : 5000,
								success:function(data){	
									$('#event_result').html('Old : ' + data.old + '&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Renamed : ' + data.renamed);
								}		
							});
						}); */
					}
